...
export default class News extends React.Component {
  ...
  render() {
    const { i, isLoading, articles } = this.state;
    return (
      <React.Fragment >
        <div key={i} className="newsData">
          {!isLoading ? (
            articles.map(article => {
              const { date, title, url, author, media, image } = article;
              return (
                <div key={i} id="newsData" className="news">
                  ...
                  <p className="description" style={{fontWeight: 'Bold'}}>{media}
                      <a href={url} target="_blank" rel="noreferrer">
                        <button style={{marginLeft: '5px'}} className="btn-klik">Read Detail</button></a></p>
                </div>
              );
            })
          ) : (
